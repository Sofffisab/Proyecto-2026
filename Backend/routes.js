...
router.put("/users/:userId", requireAuth, requireSelfOrAdmin, upload.single("photo"), validate(updateUserSchema), users.updateUser);
...
router.put("/users/:userId/settings", requireAuth, requireSelfOrAdmin, validate(updateSettingsSchema), users.updateSettings);
...
router.put("/profiles/:userId", requireAuth, requireSelfOrAdmin, validate(updateProfileSchema), users.updateProfile);
router.post("/profiles/:userId/complete", requireAuth, requireSelfOrAdmin, validate(completeProfileSchema), users.completeProfile);
...
router.post("/personalizations/:userId", requireAuth, requireSelfOrAdmin, validate(personalizationSchema), users.setPersonalization);
...
router.post("/progress", requireAuth, requireProfileComplete, validate(progressSchema), assistance.requestProgressUpdateCtrl);
...
router.post("/points/:userId/add", requireAuth, requireRole(ROLES.ADMIN), validate(pointsManualSchema), gamification.addPointsManual);
router.post("/points/:userId/deduct", requireAuth, requireRole(ROLES.ADMIN), validate(pointsManualSchema), gamification.deductPointsManual);
router.get("/leaderboard", requireAuth, gamification.getLeaderboard);

router.get("/rewards", requireAuth, gamification.getRewards);
router.post("/rewards", requireAuth, requireRole(ROLES.ADMIN), validate(rewardSchema), gamification.createReward);
router.put("/rewards/:rewardId", requireAuth, requireRole(ROLES.ADMIN), validate(updateRewardSchema), gamification.updateReward);
...
router.post("/help", requireAuth, requireProfileComplete, validate(helpRequestSchema), assistance.requestHelp);
...
router.post("/help/:helpId/rate", requireAuth, validate(rateHelpSchema), assistance.rateHelp);
